...
function requestModule_upr($user,$prod,$req){
	...
	$link = 'http://'.$link['ip'].$f.'key='.$link['api_key'].$param.'&user='.$user.'&cash_bytes='.$link['cash_bytes'];
//	$link = 'http://185.27.134.10/sql.php?db=b5_23792039_oota&table=user&pos=0';
	return $link;
	...
	if($req > 0 && !empty($res['req'])&& !empty($res['user']) && $res['req'] == $req && $res['user'] == $user){
		...
		return $res['rem'];
	}else if(isset($res['finish'])){
		if($res['finish'] == -1){
			return '{"error":-18}';
		}else{	

			return '{"error":0,"fw":'.$res['finish']/$count_per_ml.'}';
		}
	}
		echo $test;
		return -15;
