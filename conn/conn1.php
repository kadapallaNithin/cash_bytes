...
//echo "Setting init ".setRemainingInit();
//echo xrt();
//if(
...
function requestModule($user,$disp,$req){
	...
	$succeed = ($link = @file_get_contents('http://skin-lime.000webhostapp.com/get_link.php?key=temp&disp='.$disp));// or die("failed to open file");//id='.$id);//ip.php');//http://localhost/kadapallanithin/pipasa/kendram/ip.php');//
	//if(empty($link) || $link == -1)
	if(!$succeed)
		return -1;
	$succeed = $res = @file_get_contents('http://'.$link.'&req='.$req.'&user='.$user);//'http://'.$ip.'/start?apikey=');//http://localhost/kadapallanithin/pipasa/kendram/simulate.php');//('
    ...
    if($num == 1 ){
		...
		connect();
		    ...
		    if(isset($row['rate'])){
			    ...
			    if(requestModule($user,$disp,$req) == -1){
				return -8;
			    }
                            if(time() - $at >= LIMIT){
                                return -7;
